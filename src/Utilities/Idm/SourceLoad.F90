...
contains
...
#else
      write (errmsg, '(a)') &
        'Cannot load package inputs. NetCDF input file provided &
...
#else
      write (errmsg, '(a)') &
        'Cannot open input file. NetCDF input file provided &
        &but NetCDF libraries are not available.'
      call store_error(errmsg)
      call store_error_filename(modelfname)
#endif
...
#if defined(__WITH_NETCDF__)
      write (errmsg, '(a)') &
        'NetCDF Model name files not currently supported.'
      call store_error(errmsg)
      call store_error_filename(simfile)
#else
      write (errmsg, '(a)') &
        'Cannot load name file. NetCDF input file provided &
        &but but NetCDF libraries are not available.'
      call store_error(errmsg)
      call store_error_filename(simfile)
#endif
    case default
  ...
  subroutine create_nc_context(modeltype, component_type, modelname, &
    ...
    if (nc_fname /= '') then
      ...
      !
      ! -- add NETCDF packages to context
...
#else
      write (errmsg, '(a)') &
        'Cannot load model packages. NetCDF input file &
        &specified in model name file but &
  ...
  subroutine set_nc_pkglist(component_type, modelfname, pkglist, &
    ...
    integer(I4B) :: m, n
    !
    ! -- add NETCDF packages to context list
    do n = 1, size(pkglist)
      ...
      do m = 1, pkglist(n)%pnum
        ...
        if (package_source_type(pkglist(n)%filenames(m)) == 'NETCDF4') then
          ...
          if (idm_integrated(component_type, pkglist(n)%subcomponent_type)) then
            ...
            errmsg = 'NetCDF is unsupported for package type "'// &
  ...
  !> @brief set model netcdf input filename
  !<
        ...
        if (package_source_type(pkglist(n)%filenames(m)) == 'NETCDF4') then
          if (nc_fname == '') then
            ...
            errmsg = 'Multiple *.nc model input files detected in packages &
                     &block. Only one model NetCDF input supported.'
            call store_error(errmsg)
            call store_error_filename(modelfname)
          end if
        end if
