...
contains
  ...
  subroutine load_modelnam(mtype, mfname, mname, iout)
    ...
    use IdmMf6FileModule, only: model_load
    ...
    select case (source_type)
    case ('MF6FILE')
      call model_load(idm_component_type(mtype), mtype, mfname, mname, iout)
    case default
    end select
    !
    ! -- return
  ...
  subroutine load_simnam()
    ...
    if (lexist) then
      ...
      call input_load(simfile, 'NAM6', 'SIM', 'NAM', 'SIM', 'NAM', simfile, iout)
    end if
    !
    ! -- return
    return
