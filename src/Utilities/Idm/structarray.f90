module StructArrayModule
  ...
  subroutine mem_create_vector(this, icol, vartype, nrow, name, memoryPath, preserve_case)
    ...
    !character(len=LINELENGTH), dimension(:), pointer, contiguous :: cstr1d
    ...
    case ('STRING')
      !allocate_charstr1d(acharstr1d, ilen, nrow, name, mem_path)
  ...
  subroutine add_vector_str1d(this, icol, str1d, preserve_case)
    class(StructArrayType) :: this
    integer(I4B), intent(in) :: icol
    !character(len=LINELENGTH), dimension(:), pointer, contiguous, intent(in) :: str1d
    type(CharacterStringType), dimension(:), pointer, contiguous, intent(in) :: str1d
  ...
  subroutine read_from_parser(this, parser, iout)
    ...
    use IdmLoggerModule, only: idm_log_type
    class(StructArrayType) :: this
    ...
    do i = 1, this%nrow
      ...
      do j = 1, this%ncol
        ...
        case (3)
          !call parser%GetString(this%struct_vector_1d(j)%str1d(i), (.not. this%struct_vector_1d(j)%preserve_case))
          call parser%GetString(str1d, (.not. this%struct_vector_1d(j)%preserve_case))
    ...
    do j = 1, this%ncol
        select case (this%struct_vector_1d(j)%memtype)
        case (1)
          call idm_log_type(this%struct_vector_1d(j)%int1d, this%struct_vector_1d(j)%varname, this%struct_vector_1d(j)%memoryPath, &
