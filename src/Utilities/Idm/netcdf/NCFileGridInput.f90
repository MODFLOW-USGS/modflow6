...
module NCFileGridInputModule
  ...
  use LoadNCFileModule, only: NETCDF_MAX_DIM, nf_verify
...
contains
  ...
  subroutine bndgrid_init(this, mf6_input, component_name, component_input_name, &
    ...
    this%iiper = 0
    !
    call this%bound_context%create(this%mf6_input, this%readasarrays)
    !
  ...
  subroutine rp_array(mf6_input, ncpkg, param_names, param_reads, &
    ...
    ! -- local
    type(InputParamDefinitionType), pointer :: idt
    integer(I4B) :: iparam, iblock, varid, ncelldim
    character(len=LINELENGTH) :: tagname
    !
    ! -- set ncelldim
    ncelldim = size(bndctx%mshape)
    !
  ...
  subroutine load_var(mf6_input, param_names, param_reads, ncid, &
                      idt, varid, iiper, ncelldim, nc_fname, iout)
    use netcdf
    ...
    integer(I4B), intent(in) :: iiper
    integer(I4B), intent(in) :: ncelldim
    character(len=*), intent(in) :: nc_fname
    integer(I4B), intent(in) :: iout
    ! -- local
    ...
    if (load_idx > 0) then
      ...
      case ('INTEGER1D')
        call load_int1d(mf6_input, ncid, param_names, param_reads, &
                        idt, dimlens, varid, load_idx, ncelldim, iout)
        !
      case ('DOUBLE1D')
        ...
        !
      case ('DOUBLE2D')
        call load_dbl2d(mf6_input, ncid, param_names, param_reads, &
                        idt, dimlens, varid, load_idx, ncelldim, iout)
      case default
        errmsg = 'IDM unimplemented. NCFileGridInput::load_var &
                 &datatype='//trim(idt%datatype)//', var='//trim(idt%tagname)
        call store_error(errmsg)
  ...
  subroutine load_int1d(mf6_input, ncid, param_names, param_reads, &
    ...
    type(ReadStateVarType), dimension(:), allocatable :: param_reads
    type(InputParamDefinitionType), pointer, intent(in) :: idt
    integer(I4B), dimension(NETCDF_MAX_DIM), intent(in) :: dimlens
    integer(I4B), intent(in) :: varid
    ...
    if (ncelldim == 3) then
      ...
      deallocate (int3d)
      !
    else if (ncelldim == 2) then
      ! -- allocate local array
      allocate (int2d(dimlens(1), dimlens(2)))
      !
      ! -- read data into temporary
      ...
      deallocate (int2d)
      !
    else if (ncelldim == 1) then
      errmsg = 'Array based input not supported for DISU grids, package="'// &
  ...
  subroutine load_dbl1d(mf6_input, ncid, param_names, param_reads, &
    ...
    if (ncelldim == 3) then
      ...
      ! -- allocate local array
      allocate (dbl2d(dimlens(1), dimlens(2)))
      !
      ! -- read data into temporary
  ...
  subroutine load_dbl2d(mf6_input, ncid, param_names, param_reads, &
                        idt, dimlens, varid, load_idx, ncelldim, iout)
    use netcdf
    use ArrayHandlersModule, only: ifind
    ! -- dummy
    ...
    if (ncelldim == 3) then
      ...
      !
      ! -- TODO
      errmsg = 'IDM unimplemented. AUXVAR not currently supported &
               &for DISV array input.'
      call store_error(errmsg, .true.)
      !
    else if (ncelldim == 1) then
      errmsg = 'Array based input not supported for DISU grids, package="'// &
