...
contains
  ...
  function load_charstr1d(mf6_input, ncid, idt, mshape, dimlens, &
    ...
    !
    do n = 1, size(cstr1d)
      charstr1d(n) = reformat_input_str(cstr1d(n), LENBOUNDNAME)
      if (charstr1d(n) == '') then
        nbound = n - 1
        exit
      end if
  ...
  function reformat_input_str(input_str, len) result(str)
    use InputOutputModule, only: lowcase, upcase
    ...
    ! -- return
    return
  end function reformat_input_str
