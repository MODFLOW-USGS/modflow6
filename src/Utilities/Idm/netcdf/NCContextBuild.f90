...
module NCContextBuildModule
  ...
  use LoadNCFileModule, only: NETCDF_MAX_DIM, NETCDF_ATTR_STRLEN, nf_verify
...
contains
  ...
  !<
  function nc_package(nc_context, pkgtype, comp_type, subcomp_type, &
    ...
    nullify (ncpkg)
    !
    if (nc_context%has_package(comp_name, subcomp_name, idx)) then
      ncpkg => nc_context%get(idx)
  ...
  subroutine add_package_var(nc_context, modelname, varid, iout)
    ...
    if (nf90_get_att(ncid, varid, "mf6_input", &
      ...
      !
      ! -- format: <pkgtype>:<component-name>/<subcomponent-name>/<tagname>
      ...
      !
      ncpkg => nc_package(nc_context, pkgtype, nc_context%component_type, &
  ...
  function verify_global_attr(nc_context) result(mname)
    use InputOutputModule, only: lowcase, upcase
    type(NCModelInputsType), intent(inout) :: nc_context
    ! -- return
    character(len=LENCOMPONENTNAME) :: mname
    ! -- local
    !character(len=LENPACKAGETYPE) :: modeltype
    character(len=NETCDF_ATTR_STRLEN) :: modeltype
    character(len=NETCDF_ATTR_STRLEN) :: modelname
    !
