...
contains
  ...
  subroutine tsp_cr(this, filename, id, modelname, macronym, indis)
    ...
    logical(LGP) :: found_list, found_print_input, found_print_flows, &
                    found_save_flows, found_under_relaxation, found_newton
    !
    ! -- Assign values
    ...
    call mem_set_value(lst_fname, 'LIST', input_mempath, found_list)
    call mem_set_value(this%iprpak, 'PRINT_INPUT', input_mempath, &
                       found_print_input)
    call mem_set_value(this%iprflow, 'PRINT_FLOWS', input_mempath, &
                       found_print_flows)
    call mem_set_value(this%ipakcb, 'SAVE_FLOWS', input_mempath, found_save_flows)
    !
    ! -- create the list file
    call this%create_lstfile(lst_fname, filename, found_list)
    !
    ! -- activate save_flows if found
    if (found_save_flows) then
      this%ipakcb = -1
    end if
    !
    ! -- log set options
    if (this%iout > 0) then
      ...
      !
      if (found_newton) then
        write (this%iout, '(4x,a)') &
          'NEWTON-RAPHSON method enabled for the model.'
        if (found_under_relaxation) then
