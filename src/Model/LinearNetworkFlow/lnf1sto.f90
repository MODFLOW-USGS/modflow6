...
! ------------------------------------------------------------------------------
    ! -- dummy
    type(LnfStoType), pointer :: stoobj
...
! ------------------------------------------------------------------------------
    ...
    ! -- dummy
    class(LnfStoType)                       :: this
    class(LnfDislType), pointer, intent(in) :: dis
...
! ------------------------------------------------------------------------------
    ...
    character(len=*), parameter :: fmtsperror =                                &
      &"('DETECTED TIME STEP LENGTH OF ZERO.  Lnf STORAGE PACKAGE CANNOT BE ', &
...
! ------------------------------------------------------------------------------
    ...
    do n = 1, this%disl%nodes
      ...
      !  call this%thksat(n, hold(n), bbot, sato)
      !  call this%thksat(n, hnew(n)+DEM4, bbot, dsat)
      !  dsat = (dsat - satn) / DEM4
      !  if(dsat <= 1.0e-7) dsat = 1.0e-7
        ! --
      !  rterm = rho2 * (satn - sato)
      !  drterm = rho2 * dsat
      !  amat(idxglo(idiag)) = amat(idxglo(idiag)) - drterm
      !  rhs(n) = rhs(n) + rterm - drterm * hnew(n)
        ! -- add compressible storage that uses saturation to ramp it up 
      !  rterm = rho1 * satn 
      !  amat(idxglo(idiag)) = amat(idxglo(idiag)) - rterm
      !  rhs(n) = rhs(n) - rterm * hold(n)
      end if
    end do
    !
    ! -- Return
