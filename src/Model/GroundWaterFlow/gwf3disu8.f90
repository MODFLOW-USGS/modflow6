module GwfDisuModule
  ...
  use ConstantsModule, only: LENMODELNAME, LINELENGTH, LENMEMPATH, LENVARNAME, &
                             DZERO, DONE
...
! ------------------------------------------------------------------------------
    ...
    if (inunit > 0) then
      ...
      call idm_load(dis%parser, 'DISU6', '', 'GWF', 'DISU', name_model, 'DISU', &
...
! ------------------------------------------------------------------------------
    ...
    call iac_to_ia(iac, disext%con%ia)
    do n = 1, nja
...
! ------------------------------------------------------------------------------
  ...
  subroutine gen_cellvert_arrays(this, icell2d, ncvert, icvert)
    ! -- modules
    use SimModule, only: count_errors, store_error
...
! ------------------------------------------------------------------------------
    ...
    ! -- Return
    return
  end subroutine gen_cellvert_arrays
...
! ------------------------------------------------------------------------------
    ...
    if (associated(icell2d) .and. associated(ncvert) &
      ...
      call this%gen_cellvert_arrays(icell2d, ncvert, icvert)
    else
