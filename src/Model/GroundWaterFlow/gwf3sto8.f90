...
! ------------------------------------------------------------------------------
    ...
    !
    ! -- reset input context pointer to reallocated setting
    call mem_setptr(storage_setting, 'SETTING', this%input_mempath)
    !
    write (this%iout, '(//,1x,a)') 'PROCESSING STORAGE PERIOD DATA'
    !
    if (size(storage_setting) == 1) then
      !
      ! -- set period storage state
      storage_state = storage_setting(1)
      !
      ...
      !
    else if (size(storage_setting) > 1) then
      write (errmsg, '(a,i0)') &
        'A single STORAGE input setting is expected each period, found: ', &
        size(storage_setting)
      call store_error(errmsg)
      call store_error_filename(this%input_fname)
    end if
    !
