...
def get_model(idx, dir):
    ...
    for irno in range(ncol):
        ...
        if irno in [0, ncol - 1]:
            ncon = 1
        cellid = (0, 0, irno)
        t = (irno, cellid, rlen, rwid, rgrd, rtp, rbth, rhk, rman, ncon,
             ustrf, ndv, denseaux, concaux)
        pak_data.append(t)
...
def eval_results(sim):
    ...
    for n in range(6):
        hsfr = stage[n]
        hgwf = head[n]
        rhogwf = 1000. + 0.7 * cgwf[n]
        rhosfr = 1000. + 0.7 * csft[n]
        avgdense = .5 * rhogwf + .5 * rhosfr
        elevgwf = -0.5
        elevsfr = 0.0
        avgelev = 0.5 * elevgwf + 0.5 * elevsfr
        avghead = 0.5 * hgwf + 0.5 * hsfr
        qcalc = cond * (hsfr - hgwf)
        qcalc += cond * (avgdense / 1000. - 1.) * (hsfr - hgwf)
        qcalc += cond * (avghead - avgelev) * (rhosfr - rhogwf) / 1000.
        qsim = -qsfrgwfsim[n]
        #print(n, hsfr, hgwf, rhosfr, rhogwf, qcalc, qsim)
        #if not np.allclose(qcalc, qsim):
        #    print('reach {} flow {} not equal {}'.format(n, qcalc, qsim))
        assert np.allclose(qcalc, qsim), 'reach {} flow {} not equal {}'.format(n, qcalc, qsim)
