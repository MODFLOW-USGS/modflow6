...
ws = os.path.join('temp', 'examples', 'vsc-sfr01')
...
#
def build_model(idx, sim_folder='vsc_wSFR'):
    print("Building model...{}".format(sim_folder))

    # generate names for each model
    name = "vsc"
    gwfname = "gwf-" + name + str(idx) + "-sfr"
    gwtname = "gwt-" + name + str(idx) + "-sfr"
    ...
    for i in np.arange(ncol):
        rech[0, i] = rech_rate_lo + \
                    (rech_rate_hi - rech_rate_lo) / ncol*i
