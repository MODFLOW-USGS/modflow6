...
def build_model(ws, name, uzf=False):
    ...
    sto = flopy.mf6.ModflowGwfsto(gwf, sy=sy, ss=ss, transient={0: True},
                                  iconvert=1)
    if uzf:
        uzf = flopy.mf6.ModflowGwfuzf(gwf, simulate_gwseep=True,
                                      packagedata=uzf_pd, print_input=True)
        uzf.obs.initialize(filename='{}.uzf.obs'.format(name), digits=20,
                           print_input=True, continuous=uzf_obs)
    else:
        drn = flopy.mf6.ModflowGwfdrn(gwf, auxiliary=['ddrn'],
                                      auxdepthname='ddrn',
                                      stress_period_data=drn_spd,
                                      print_input=True)
        drn.obs.initialize(filename='{}.drn.obs'.format(name), digits=20,
                           print_input=True, continuous=drn_obs)
...
def eval_disch(sim):
    ...
    fpth = os.path.join(sim.simpath,
                        '{}.disc.cmp.out'.format(os.path.basename(sim.name)))
    f = open(fpth, 'w')
...
def test_mf6model():
    ...
    for idx, dir in enumerate(exdirs):
        if is_CI and not continuous_integration[idx]:
            continue
        yield test.run_mf6, Simulation(dir, exfunc=eval_disch,
...
def main():
    ...
    # run the test models
    for idx, dir in enumerate(exdirs):
        sim = Simulation(dir, exfunc=eval_disch, exe_dict=replace_exe,
                         idxsim=idx)
        test.run_mf6(sim)
    return
