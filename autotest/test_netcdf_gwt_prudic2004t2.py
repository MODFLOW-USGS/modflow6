...
def build_models(idx, test):
    ...
    if transport:
        ...
        ic = flopy.mf6.ModflowGwtic(gwt, export_array_netcdf=True, strt=0.0)
        mst = flopy.mf6.ModflowGwtmst(gwt, porosity=0.3)
        adv = flopy.mf6.ModflowGwtadv(gwt, scheme="TVD")
        dsp = flopy.mf6.ModflowGwtdsp(
            gwt, export_array_netcdf=True, alh=20.0, ath1=2, atv=0.2
        )
        sourcerecarray = [()]
        ...
        for j in range(pd[i][1]):
            ...
            for l in range(nlay):
                assert np.allclose(
                    ...
                    xds[f"concentration_l{l+1}"][timestep, :].fillna(1.00000000e+30).data,
