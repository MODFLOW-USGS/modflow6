...
class TestFramework:
    ...
    def _compare_budgets(self, msgall, extensions="cbc"):
        ...
        ipos = 0
        for idx, (fpth0, fpth1) in enumerate(zip(files0, files1)):
            success_tst, msg = self._compare_budget_files(
                ipos,
                extension,
                fpth0,
                fpth1,
    ...
    def run_all(self):
        """
        Run simulations and assert successful termination
        """

        # get expected input/output files
        namfile_path = self.workspace / "mfsim.nam"
        mf6inp, mf6outp = get_mf6_files(namfile_path)
        self.outp = mf6outp
        ...
        if self.parallel:
            ...
            try:
                success, buff = self._run_parallel(
                    exe,
                )
