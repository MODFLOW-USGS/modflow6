...
surf_Q_in = [[8.64, 0.], [8640.0, 0.], [8640.0, 0.], [8640.0, 0.]]  # 86400 m^3/d = 1 m^3/s = 35.315 cfs
...
def process_line(line):
    ...
    if any("=" in itm and len(itm) > 1 for itm in m_arr):
        m_arr = [float(itm.split("=")[-1]) if len(itm.split("=")) > 1 else itm for itm in m_arr]
        nm = m_arr[-2]
    else:
        nm = m_arr[-3]
    val = m_arr[-1]
...
K_therm_strmbed = [1.5, 1.75, 2.0]  # Thermal conductivity of the streambed material ($W/m/C$)
...
def build_model(idx, dir):
    ...
    strm_up = 100.25
    strm_dn = 99
    slope = (strm_up - strm_dn) / ((ncol - 1) * delr) / 10  # divide by 10 to further reduce slop
    ustrf = 1.0
    ndv = 0
    strm_incision = 1.0
...
def eval_results(sim):
    ...
    fname1 = os.path.join(sim.simpath, fname1)
    ...
    msg3 = "There should be a decreasing temperatures trend in " \
           "downstream direction owing to conductive losses"
    msg4 = "There should be an increasing temperature trend in " \
    ...
    if name[-1] == 'n':   # no gw/sw convective exchange, simulates conductive exchange only
        ...
        if sfe_temps[0, 0, 0, 0] > gw_temps[0, 0, 0, 0]:
            ...
            slp = trenddetector(np.arange(0, sfe_temps.shape[-1]), sfe_temps[0, 0, 0, :])
            assert slp < 0.0, msg3
