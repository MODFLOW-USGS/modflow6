...
\item The GWT Model can optionally submit a preferred time step size based on a user-specified maximum allowable change in concentration (ATS\_DCMAX specified in the GWT name file).  If specified by the user, the GWT Model will calculate the maximum allowable time step that will result in a maximum change in concentration that does not exceed this ATS\_DCMAX value.
\end{itemize}
