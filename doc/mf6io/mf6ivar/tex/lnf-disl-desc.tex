...
\begin{description}
\item \texttt{nodes}---is the number of linear features.
...
\item \texttt{fdc}---is the fractional distance to the cell center. FDC is relative to the first vertex in the ICVERT array. In most cases FDC should be 0.5, which would place the center of the cell in the middle of the cell. If the value of FDC is 1, the cell center would located at the last vertex. FDC values of 0 and 1 can be used to place the node at either end of the reach which can be useful for cells with boundary conditions.
