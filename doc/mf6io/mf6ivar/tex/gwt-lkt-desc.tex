...
\item \texttt{flowbudgetfile}---name of the binary lake flow budget file to read as input for the LKT Package.  This budget file can be created by running a separate GWF Model with the Lake Package and saving the lake flows to the budget file.
...
\item \textcolor{blue}{\texttt{inflow}---real or character value that defines the concentration of inflow $(ML^{-3})$ for the lake. Value must be greater than or equal to zero. If the Options block includes a TIMESERIESFILE entry (see the ``Time-Variable Input'' section), values can be obtained from a time series by entering the time-series name in place of a numeric value.}
