...
\lstinputlisting[style=blockdefinition]{./mf6ivar/tex/gwf-sfr-tables.dat}
...
Streamflow Routing Package observations include reach stage and all of the terms that contribute to the continuity equation for each stream reach. Additional SFR Package observations include the sum of inflows from upstream reaches, from mover terms (\texttt{upstream-flow}) and downstream outflow from a reach prior to diversions and the mover package (\texttt{downstream-flow}), the wetted perimeter of a reach (\texttt{wetted-perimeter}), and the cross-sectional area of a reach (\texttt{area}). \texttt{wetted-perimeter} and \texttt{area} observations for a reach are constant unless cross-section data is specified for a reach in the optional \texttt{TABLES} block. The data required for each SFR Package observation type is defined in table~\ref{table:gwf-sfrobstype}. Negative and positive values for \texttt{sfr} volumetric flow observations represent a loss from and gain to the GWF model, respectively. For all other volumetric flow terms, negative and positive values represent a loss from and gain from the SFR package, respectively.
...
\newpage
\subsection{SFR Reach Cross-Section Table Input File}
SFR reach cross-section tables containing station and elevation cross-section data can be specified for individual reaches.  SFR cross-section tables are specified by including file names in the TABLES block of the SFR Package.  These file names correspond to a SFR cross-section table input file.  The format of the SFR cross-section table input file is described here.
...
\subsubsection{Explanation of Variables}
\begin{description}
\input{./mf6ivar/tex/utl-sfr-tab-desc.tex}
\end{description}
