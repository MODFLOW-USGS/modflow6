...
Any of these examples support writing the read array to a output array file with the ECHO keyword.  The generated file follows a naming convention derived from the model and package names as well as the name of the ECHO variable.  For example, if the model ``mymodel'' STRT variable in the package ``myicpkg'' included the ECHO option, the generated echo file would be named ``MYMODEL-MYICPKG.strt''.  The ECHO option is supported for many ``GRIDDATA'' input varaibles, whether read in a LAYERED format or as the entire model grid.
