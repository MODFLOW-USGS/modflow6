...
All of the previous examples also support writing the input array values to an output file with the ECHO keyword.  The generated file follows a naming convention derived from the model and package names as well as the name of the ECHO variable.  For example, the IC package STRT variable in model ``mymodel'' and package ``myicpkg'' would be written to a file named ``MYMODEL-MYICPKG.strt''.  The ECHO option is supported for many ``GRIDDATA'' input varaibles, whether read in a LAYERED format or as the entire model grid.  Currently, there is no way to specify print formating codes for the written arrays and as such numeric data may appear in mixed formats.
