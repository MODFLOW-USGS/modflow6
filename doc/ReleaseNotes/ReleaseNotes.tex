...
\item In some cases the table writer, such as the one used to write the lake stage table, would allow numbers in adjacent columns to touch.  Made a slight modification to leave an extra space when writing double precision values so that the number does not touch with the number in the preceding column.
\end{itemize}
