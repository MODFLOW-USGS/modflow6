...
\item Fixed an issue in the LAK package when NOUTLETS is not specified or specified to be zero n the DIMENSIONS block and an OUTLETS block is specified. Previously, this did not cause an error condition in the LAK package but could cause segmentation fault error conditions in the MVR package if OUTLETS are specified and providers. An error condition will occur if the OUTLETS block is present and  NOUTLETS is not specified or specified to be zero in the DIMENSIONS block.
\end{itemize}
