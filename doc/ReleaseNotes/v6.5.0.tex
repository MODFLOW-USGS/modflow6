...
% after a release has just been made.
	...
	\begin{itemize}
		...
		\item An SFR channel defined with the n-point cross-section option was calculating the wetted cross-sectional area incorrectly. The cross sectional area for the area of a triangle was being calculated as one-half multiplied by the depth of the channel, as opposed to one-half multiplied by the base width multiplied by the height. With this fix, a few spot differences showed up in the the autotests, requiring some minor tweaks to a few set answers.  Calculated stages in SFR reaches that use the n-point cross-section option, and therefore groundwater surface-water exchange amounts, will change in existing models as a result of this fix.  Changes are not expected to be large.
	%	\item xxx
	\end{itemize}
