...
% after a release has just been made.
	...
	\begin{itemize}
		...
		\item An SFR channel defined with the n-point cross-section option was calculating the wetted cross-sectional area incorrectly. The cross sectional area for the area of a triangle was being calculated as one-half multiplied by the depth of the channel, as opposed to one-half multiplied by the base width multiplied by the height. This fix led to a related fix for Manning equation.  When the sides of a channel were vertical, the code was not taking into account the vertically wetted length in the wetted perimeter calculation.  With this fix, a number of the autotests needed updating.  Calculated stages in SFR reaches, and therefore groundwater surface-water exchange amounts, will change in existing models as a result of this fix.
	%	\item xxx
	\end{itemize}
