...
% has just been made.
	...
	\begin{itemize}
		...
		\item The wrong sign convention was being applied to GWET when the SQUARE\_GWET option was invoked in the UZF options block.  As a result, the calculated GWET was being added to the water table.  Additionally, GWET was being reported as an inflow in the water budget even though it doesn't make physically sense.  The applied fix ensures that GWET is removed from the water table and as a result the GWET values are accumulated as outflows in the budget table.
	%	\item xxx
