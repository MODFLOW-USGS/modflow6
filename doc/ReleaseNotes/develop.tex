...
% after a release has just been made.
	...
	\begin{itemize}
		...
		\item Previously the PRT model's default behavior was to track particles until termination, as with MODPATH 7 stop time option 'extend'. Under extended tracking, the program may never halt if particles enter a cycle in the flow system. This PR changes the default behavior to the equivalent of MP7 stop time option 'final', terminating particles when the simulation ends unless new PRP keyword option EXTEND is provided. This is meant to provide a (stronger) guarantee that the program halts under default settings.
	%	\item xxx
	%	\item xxx
	\end{itemize}
