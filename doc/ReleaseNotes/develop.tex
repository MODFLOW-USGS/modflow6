...
% after a release has just been made.
	...
	\begin{itemize}
		...
		\item A bad pointer reference has been fixed in the PRT model's "pass-to-bottom" tracking method. Depending on the combination of platform and compiler used to build the program, this could result in crashes (e.g. divide by zero errors) or in silent failures to properly pass particles downward between vertically adjacent cells, leading to early termination. The latter could occur as a consequence of undefined behavior which prevented detection of situations when a particle should exit a cell through its bottom face.
	%	\item xxx
	\end{itemize}
