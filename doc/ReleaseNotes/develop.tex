...
% after a release has just been made.
	...
	\underline{EXAMPLES}
	\begin{itemize}
		\item A new example compares GWE output to an analytical solution published in the peer reviewed literature.  Energy is added to a confined aquifer with an initial temperature of 0.0$^{\circ}C$ at a constant rate of 100 $\frac{W}{m}$ using the energy source loading (ESL) package.  An output plot showing isotemperature contours for both GWE and the analytical solution qualitatively demonstrates that GWE adequately reproduces the analytical solution.
	%	\item xxx
	%	\item xxx
	\end{itemize}
